...
DEVICE_ID = "a30ceb56ee9c2ed183qrqz"
LOCAL_KEY =  r"c~Ov[~zh[i'N!pRU"
IP_ADDRESS = "192.168.137.128" # <--- ⚠️ อย่าลืมใส่เลข IP ล่าสุดที่สืบมาจากแอปนะครับ

# --- 2. ตั้งค่าการทดลอง (ปรับเป็น 1 ชั่วโมง) ---
LIMIT_MINUTES = 60        # ตั้งเวลาตัดการทำงานที่ 60 นาที (1 ชั่วโมง)
interval = 1              # เก็บข้อมูลทุก 1 วินาที
...
# ตัวแปรคำนวณพลังงานด้วยวิธีทางวิศวกรรม (พื้นที่ใต้กราฟ)
...
try:
    with open(f"Research_Data_1Hr_{filename_prefix}.csv", mode='w', newline='') as file:
        ...
        while True:
            ...
            elapsed_min = elapsed_sec / 60.0 # แปลงเวลาเป็นนาที
            
            # คำนวณ Delta T ของเวลาในแต่ละลูปจริงเพื่อความแม่นยำสูง
            dt_seconds = current_time - last_loop_time
            last_loop_time = current_time
            
            # ตรวจสอบเมื่อครบ 1 ชั่วโมง (60 นาที)
            ...
            if data and 'dps' in data:
                ...
                # --- การคำนวณทางวิศวกรรมไฟฟ้าและอุณหภูมิ ---
                ...
                # คำนวณพลังงานสะสมจริงรายวินาที (Numerical Integration)
                calculated_energy_kwh += power_kw * (dt_seconds / 3600.0)

                # บันทึกค่าเข้า Array สำหรับวาดกราฟ
                ...
                # บันทึกลง Excel (เวลาบันทึกเป็นหน่วยนาที)
                writer.writerow([
                    round(elapsed_min, 2), 
                    voltage_v, 
                    current_amp, 
    ...
    # --- 3. ส่วนวาดกราฟวิจัย (เมื่อรันครบ 1 ชั่วโมง) ---
    if not times_min:
    ...
    # Downsample ทุก 10 วินาที (index 0, 10, 20, ...) เพื่อให้กราฟไม่หนักเกินไป
    step = 10
    t   = times_min[::step]
    p   = powers[::step]
    tp  = temps[::step]
    ...
    # ชั้นที่ 1: กราฟกำลังไฟ และ อุณหภูมิเครื่องชาร์จ
    color1 = 'tab:blue'
    ...
    # ชั้นที่ 2: กราฟแรงดันไฟฟ้า และ กระแสไฟฟ้า
    color3 = 'tab:green'
